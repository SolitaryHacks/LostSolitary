...
Finish implementing the story into the game. We worked hard to build a narrative for the user and if given enough time later in the future, have this game prepared and ready for the user. Another implementation we wish to meet later on for the story is for the models to display an avatar wireframe in which we can control the models for the users display instead of staying in their default pose. We also want to give more additional visuals such as sound effects and interactions for the users to have besides just chat control.
